...
export class CuponPage implements OnInit {
  ...
  loading: any;
  cupon = new RespuestaCupones();
  @ViewChild(IonContent, {static: true}) content: IonContent;

  constructor( private loadingCtrl: LoadingController,
    public toastCtrl: ToastController,
    private cuponesServ: CuponesService,
  ...
  ngOnInit() {
    ...
    if ( id !== 'nuevo' ) {
        this.cuponesServ.getCupon( id )
          .subscribe( (resp: RespuestaCupones)  => {
            this.cupon = resp;
            this.cupon.idCupon = id;
    ...
    guardar( formulario: NgForm ) {
      ...
      if ( this.cupon.idCupon ) {
        // Existe y lo actualizo
        peticion = this.cuponesServ.actualizarCupon( this.cupon );
        mensajeUser = 'OK! Se actualizaron los datos';
      } else {
        // No existe, agrego un documento
        peticion = this.cuponesServ.crearCupon( this.cupon );
