...
export class PlanPage implements OnInit {
  ...
  loading: any;
  plan = new RespuestaPlanes();
  @ViewChild(IonContent, {static: true}) content: IonContent;

  constructor( private loadingCtrl: LoadingController,
    public toastCtrl: ToastController,
    private planesServ: PlanesProdService,
  ...
  ngOnInit() {
    ...
    if ( id !== 'nuevo' ) {
        this.planesServ.getPlan( id )
          .subscribe( (resp: RespuestaPlanes)  => {
            this.plan = resp;
            this.plan.idPlan = id;
  ...
  guardar( formulario: NgForm ) {
    ...
    if ( this.plan.idPlan ) {
      ...
      peticion = this.planesServ.actualizarPlan( this.plan );
      mensajeUser = 'OK! Se actualizaron los datos';
    } else {
      // No existe, agrego un documento
      peticion = this.planesServ.crearPlan( this.plan );
    ...
    peticion.subscribe ( resp => {
      ...
      // Redirigo a la lista
      // this.router.navigateByUrl('/planes/', this.plan.idProducto);
      this.router.navigateByUrl(`/planes/${ this.plan.idProducto }`);
